...
def search_command():
    list.delete(0,END)
    for row in backend.search(date_text.get(),study_text.get(),reading_text.get(),pro_text.get(),walking_text.get(),Movie_text.get()):
        list.insert(END,row)

def add_command():
    backend.insert(date_text.get(),study_text.get(),reading_text.get(),pro_text.get(),walking_text.get(),Movie_text.get())

    list.delete(0,END)
    list.insert(END,(date_text.get(),study_text.get(),reading_text.get(),pro_text.get(),walking_text.get(),Movie_text.get()))

win = Tk()

win.wm_title('Basic Database')

l1 = Label(win, text='Date')
l1.grid(row=0,column=0)
l2 = Label(win, text='Study')
l2.grid(row=0,column=2)
l3 = Label(win, text='Reading')
l3.grid(row=1,column=0)
l4 = Label(win, text='Programming')
l4.grid(row=1,column=2)
l5 = Label(win, text='Walking')
l5.grid(row=2,column=0)
l6 = Label(win, text='Movie')
...
e2 = Entry(win, textvariable=study_text)
...
pro_text = StringVar()
e4 = Entry(win, textvariable=pro_text)
e4.grid(row=1,column=3)

walking_text = StringVar()
e5 = Entry(win, textvariable=walking_text)
e5.grid(row=2,column=1)

Movie_text = StringVar()
e6 = Entry(win, textvariable=Movie_text)
